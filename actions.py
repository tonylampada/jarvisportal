...
action_runners = {
    ...
    # "updateFile": action_updateFile_lines or action_updateFile_anchors, TBD
    "exec": action_exec,
}

def exec_actions(actions):
    return [exec_action(action, True) for action in actions]

def exec_action(action, parse_arguments_json=False):
    if(parse_arguments_json):
        try:
